...
const ContentSide: React.FC<ContentSideProps> = ({ category }) => {
  ...
  // Memoizing the initialMaterials to avoid unnecessary recalculation
  const initialMaterials: StudyMaterial[] = useMemo(() => {
    return storedMaterials ? JSON.parse(storedMaterials) : [];
  }, [storedMaterials]);

  const [studyMaterials, setStudyMaterials] = useState<StudyMaterial[]>(initialMaterials);
